...
const timeAgent = new Agent({
  name: 'Time Agent',
  handoffDescription: 'Pass to Time Agent if the user query is about current time or timezones.',
  model: config.openAIModel,
  instructions: 'You provide current time information. You can answer questions and call tools when useful.',
...
const triageAgent = new Agent({
  ...
  instructions:
    [
      'You are an LLM agent running on CLI. You can answer questions and call tools when useful.',
...
triageAgent.on('agent_handoff', (ctx, nextAgent) => {
  console.debug(`\n[Agent Handoff] Handing off to ${nextAgent.name}`);
});
...
while (true) {
  ...
  try {
    const result = await run(triageAgent, thread.concat({ role: 'user', content: text }));
    stopSpinner();
